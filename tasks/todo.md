...
1. [x] **Contain the leaked key (PROD-02, urgent).** The user confirmed revoking the exposed key. Review the Default project's recent usage and $10 hard spend limit, keep credential values out of output, and replace the old value in `.env.local` and both Vercel projects.
...
4. [x] **Install a new key safely.** Validate the user-supplied single-line replacement without printing it, update the Secret in Preview and Production for both Vercel projects, and redeploy. Keep the old disclosed key revoked.
5. [x] **Verify the primary deployment end to end (PROD-01).** Rebuild Preview and confirm Ready. Vercel SSO blocked an authenticated Preview browser run, so use a disposable account on the fresh Production deployment for the live action: choose duration/focus, generate a valid preview, confirm exactly one attempt and no session or sets before Start, inspect sanitized logs, verify the public alias and $10 hard spend limit, then remove the disposable account and its scoped rows.
6. [x] **Handle attempts lost to the outage.** A read-only audit found three pre-recovery attempts across one user, who was below the rolling daily limit. Let the attempts expire naturally after 24 hours; no real-user rows need deletion.
...
8. [x] **Close with evidence.** Record deployment IDs, sanitized log outcomes, authenticated smoke results, quota outcome, and final status for PROD-01/02/03 in `context/current-issues.md` and `context/progress-tracker.md`. Deliver this report through the branch → PR → merge → local `main`/`development` sync workflow.
...
PR #20 merged as `8140279`; the malformed-key guard and safe-stage logging pass 10 focused generation checks, 50 unit tests, 56 serial PostgreSQL integration checks, lint, and production build. The user revoked the exposed key and saved a replacement, which passed the model availability check. Both Vercel projects received the replacement and fresh deployments. The primary Production test generated a valid three-exercise Push Day, creating one attempt and no workout session or sets; its disposable account and rows were removed. A sanitized scan of 13 new-deployment records found no API key, Bearer value, or generation failure. The Default project displayed a $10 hard spend limit and $0.00 spend. The second project's Clerk sign-in still works. Three pre-recovery attempts across one user can expire naturally; that user was below the daily limit. Authenticated Preview browser verification was blocked by Vercel SSO, though its rebuild passed.
...
# Mobile exercise issues — repair plan (2026-09-25)

## Specification

- **Goal:** Fix MOBILE-01, MOBILE-02, and MOBILE-03 in `context/current-issues.md` without changing set persistence, retry safety, or progression.
- **Scope:** The exercise calibration/adjustment and repetitions sheets, their shared bottom-sheet behavior, and the active round timer. No database or action contract change is expected.
- **Product decision:** A successfully logged set ends its round countdown immediately. Show a clear `Round complete` state with the time remaining frozen until Next round or Undo; release the wake lock and prevent a later time-up vibration. This supersedes the earlier rest-countdown decision in `context/progress-tracker.md` and spec 06. Failed saves leave the round active and retryable.
- **Acceptance:** On a physical iPhone with the numeric keyboard open, each field, validation message, close control, and submit action remains visible or reachable within the sheet. After a successful rep save, the displayed time and progress no longer change; failed saves and Undo retain their existing safety behavior.

## Ordered work

1. [x] **Trace the affected flows and review the plan.** Both exercise forms use `SheetContent side="bottom"` and focus a numeric input on open. The shared sheet uses `position: fixed` and `bottom: 0`; `max-h-dvh`/scrolling alone does not prove keyboard clearance on iPhone Safari. `RoundTimer` keeps an interval and wake lock until unmount or a new round. Record the prior timer-design conflict above.
2. [ ] **Reproduce and measure on iPhone Safari.** Check calibration, adjustment, and rep logging on the deployed or local app with the keyboard open. Record the visible viewport and sheet/input/button rectangles, including after keyboard animation, field switching, validation errors, closing, and reopening. Use a narrow viewport browser check for layout regressions, but treat physical keyboard evidence as the acceptance gate.
3. [x] **Fix bottom-sheet positioning for exercise forms.** Added `KeyboardAwareSheetContent` around the existing sheet for the two numeric forms. It follows `visualViewport` and limits height without editing generated UI components. Safe-area padding, other sheet sides, and the Generate sheet stay unchanged.
4. [x] **Stop the timer on successful rep logging.** Let the existing successful `logSet` result mark the round complete, sample `Date.now()` once for the frozen remaining time, stop countdown updates, release its wake lock, and suppress time-up vibration after completion. Label the state `Round complete` so a frozen `01:29` does not look active. Next round gets a fresh timer. Failed or stale saves must not show a completed round; Undo of the current set returns to Start, while Undo of an older set leaves the current round alone.
5. [ ] **Verify focused cases and document the new decision.** Check iPhone Safari in browser and installed-app modes if used; exercise both form sheets with keyboard open, short screens, portrait/landscape, decimal/numeric keyboards, validation errors, and keyboard dismissal. Check timer success before and at zero, failure/retry, Next round, Cancel, level-up, and Undo. Run focused regression checks plus lint/build. Update `context/current-issues.md`, spec 06, and `context/progress-tracker.md` with observed outcomes and the superseding timer behavior.
